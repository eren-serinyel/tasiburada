...
export async function seedCarriers(
  ...
  for (let i = 0; i < CARRIER_COMPANIES.length; i++) {
    ...
    const carrier = carrierRepo.create({
      ...
      vehicleBrand: randomFrom(['Ford', 'Mercedes', 'Renault', 'Fiat', 'Isuzu', 'MAN', 'Volvo']),
      vehicleModel: vtName,
      vehicleYear: randomInt(2015, 2023),
      vehicleCapacityM3: randomFloat(5, 80),
      profileCompletion: isVerified ? randomInt(75, 100) : randomInt(30, 60),
      isActive: true,
