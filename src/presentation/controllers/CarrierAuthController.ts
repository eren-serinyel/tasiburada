...
export class CarrierAuthController {
  ...
  login = async (req: Request, res: Response) => {
    try {
      // E-posta nakliyeci tablosunda hiç yoksa → müşteri tablosuna bak → 403
      // Şifre yanlışsa (nakliyeci bulundu ama hash eşleşmedi) → direkt 401
      const carrierExists = await this.authService.checkEmailExists(req.body.email);
      if (!carrierExists) {
        const customerRepo = new CustomerRepository();
        const customer = await customerRepo.findByEmail(req.body.email).catch(() => null);
        if (customer) {
          res.status(403).json({ success: false, message: 'Bu hesap bir müşteri hesabıdır. Lütfen "Müşteri" sekmesinden giriş yapın.' });
          return;
        }
      }
