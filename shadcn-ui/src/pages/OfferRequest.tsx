...
import { MapPin, Package, Truck, Shield, Award, MessageSquare, Star, CheckCircle2, XCircle, Info, Images } from 'lucide-react';
...
export default function OfferRequest() {
  ...
  const [form, setForm] = useState({
    ...
    date: '',
    // Step 2
  scope: (localStorage.getItem('auto_scope') as 'sehirici' | 'sehirlerarasi' | null) || '' as '' | 'sehirici' | 'sehirlerarasi', // Taşıma Kapsamı
    transportType: '', // evden-eve | ofis-tasima | parca | depolama
    placeType: '', // 2+1 ev, küçük ofis vb.
  // loadTemplate kaldırıldı
    loadType: '', // eski alan – opsiyonel
    vehicleType: '', // opsiyonel tercih
    weightKg: '', // opsiyonel – şablon seçilince tahmini kullanılacak
    floor: '',
    hasElevator: false,
    timeWindow: '', // sabah | ogle | aksam | farketmez
  ...
  serviceOptions: {} as Record<string, string[]>, // { grup: [secenekKey, ...] }
  extraServices: [] as string[], // eski alan (uyumluluk için)
    photos: [] as File[],
    note: '',
  });
  // Kapsamı otomatik belirliyoruz; kullanıcı değiştiremez

  // Taşıma tipine göre alt kategori (yer/şablon) seçenekleri (tek select)
  ...
  // Taşıma tipi → hizmet grubu eşlemesi (alt seçenekler burada gösterilecek)
  ...
  // Taşıma tipi değişince yalnızca ilgili grup altındaki seçimleri tut
  useEffect(() => {
    setForm(prev => {
  ...
  // Araç tercihi ve klasik yük türü opsiyonel; taşıma tipi + (gerekliyse) yer türü yeterli
  const canNextFrom2 = !!form.scope && !!form.transportType && (isCityFlow ? true : !!form.placeType);

  // Basit eşleştirme mantığı
  // Şablonlardan tahmini ağırlıklar (kg)
  ...
  const suitableCarriers = useMemo(() => {
    ...
    const sorted = [...list].sort((a, b) => {
      ...
      // price (baseFee): düşükten yükseğe
  ...
  // Taşıma tipinin alt seçenekleri değiştiğinde; seçenek yoksa placeType'ı temizle
  useEffect(() => {
    if (!altOptions.length && form.placeType) handleChange('placeType', '');
  }, [altOptions.length]);

  // Rota değiştiğinde kapsamı otomatik belirle ve sakla
  useEffect(() => {
  ...
  // Şehir değiştiğinde ilgili ilçeleri yükle ve ilçe seçimlerini sıfırla
  useEffect(() => {
    (async () => {
      if (form.originCity) {
        const list = await getDistrictsForCity(form.originCity);
        setOriginDistricts(list);
        // mevcut seçim listede yoksa sıfırla
  ...
  const submitStep1 = (e: React.FormEvent) => {
    ...
    // Güvence: step 1 geçişinde kapsamı hesapla ve sakla
    const autoScope: 'sehirici' | 'sehirlerarasi' = form.originCity === form.destinationCity ? 'sehirici' : 'sehirlerarasi';
  ...
  // Query param ile ön-doldurma ve direkt 3. adıma geçiş
  useEffect(() => {
    ...
    if (origin || destination || date) {
      setForm((f) => ({
        ...
        date,
        // İlçeleri boş bıraktık; kullanıcı 2. adımda ya da 1. adımda seçebilir
      }));
      // Şehirler varsa 1. adımdan 2. adıma otomatik geçiş yapalım
      setStep(2);
    }
  }, [searchParams]);
  ...
  return (
  <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 overflow-visible">
      <div className="mb-6">
      ...
      {step === 1 && (
        <form onSubmit={submitStep1} className="space-y-6">
          <Card className="shadow-sm hover:shadow transition">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><MapPin className="h-5 w-5" /> Rota Bilgileri</CardTitle>
        ...
        <form onSubmit={submitStep2} className="space-y-6">
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <CardContent className="space-y-6">
              {/* Taşıma Kapsamı, Taşıma Tipi ve Yer Türü */}
              ...
              {/* Araç Tercihi üst grid kaldırıldı; aşağıdaki Araç & Bina içinde sunuluyor */}

              {/* Klasik Yük Türü (sadece Parça Eşya için) */}
              ...
              {/* Opsiyonel Eşya Listesi kaldırıldı */}
              <Separator className="my-2" />
              {/* Zaman & Sigorta */}
              <div className="space-y-2">
              ...
              {/* Taşıma tipine göre tek grup alt seçenekler */}
              {currentServiceGroup && (
          ...
          <div className="flex justify-between gap-2">
            ...
            <div className="flex gap-2">
              <Button type="button" variant="ghost" onClick={() => setStep(3)} disabled={!canNextFrom2}>Uygunları Gör</Button>
...
// Nakliyeci kartı (zengin)
function CarrierCard({ carrier, form, onRequest, onProfile }: { carrier: Carrier; form: any; onRequest: () => void; onProfile: () => void }) {
      ...
      <CardContent>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={onProfile}>Profili Gör</Button>
          </div>
        </div>
      </CardContent>
    </Card>
