...
> **SPRINT DURUMU (4 Nisan 2026 — Güncellendi):**  
> Sprint 1–7 tamamlandı. Tüm kritik form sorunları giderildi; backend şifre validasyonu eklendi.  
> Devam eden (Post-MVP): **(A)** Sosyal giriş (Google/Apple), **(B)** OTP telefon doğrulama, **(C)** Nakliyeci operasyonel bilgilerinin localStorage'dan backend'e tam taşınması.
...
4. ~~**API URL hardcoded:**~~ ✅ **DÜZELTİLDİ** — `API_BASE_URL` `/api/v1` olarak güncellendi; proxy üzerinden çalışıyor.
5. ~~**alert() ile hata gösterimi:**~~ ✅ **DÜZELTİLDİ** — `useToast` ile toast bildirimleri kullanılıyor.
6. ~~**Şifre kuralları tutarsız:**~~ ✅ **DÜZELTİLDİ** — Backend `validatePassword` utility eklendi: min 8 karakter + büyük harf + rakam standardı; müşteri kaydı, nakliyeci kaydı ve şifre sıfırlamada uygulanıyor.
...
5. ~~**Kullanım koşulları onayı yok:**~~ ✅ **DÜZELTİLDİ** — `termsAccepted` state + checkbox + KVKK Aydınlatma Metni onayı eklendi. Submit butonu kabul edilmeden disabled.
...
2. ~~**Demo hesapları production'da görünür:**~~ ✅ **DÜZELTİLDİ** — `{import.meta.env.DEV && (...)}` ile demo hesaplar sadece geliştirme ortamında görünür.
...
2. ~~**Step 3 mock data ile çalışıyor:**~~ ✅ **DÜZELTİLDİ** — `POST /api/v1/shipments/` gerçek API çağrısı uygulandı; mock data ve mockDb kullanımı tamamen kaldırıldı.
...
> **Güncel Durum (4 Nisan 2026):** Sprint 1–7 tamamlandı. Aşağıda her satırın güncel durumu belirtilmiştir.

| # | Sorun | Etkilenen Form | Kullanıcıya Etkisi | Çözüm Zorluğu | Durum |
|---|-------|----------------|---------------------|----------------|-------|
| 1 | ~~İlan oluşturma Step 3 mock data ile çalışıyor~~ | OfferRequestForm | **Yüksek** | Orta | ✅ DÜZELTİLDİ (Post /shipments gerçek API) |
...
| 6 | ~~Müşteri kayıtta API_BASE_URL hardcoded localhost~~ | RegisterUser | **Yüksek** | Kolay | ✅ DÜZELTİLDİ |
| 7 | Login'de kullanıcı tipi seçimi zorunlu (yanlış tab = hata) | Login | **Orta** — Kullanıcı frustrasyonu | Orta | ❌ Bekliyor |
| 8 | ~~Demo hesap şifreleri açık metin~~ | Login | **Orta** | Kolay | ✅ DÜZELTİLDİ (import.meta.env.DEV koşulu) |
| 9 | ~~Nakliyeci kayıtta kullanım koşulları onayı yok~~ | RegisterCarrier | **Orta** | Kolay | ✅ DÜZELTİLDİ (KVKK checkbox eklendi) |
...
1. ~~**RegisterUser.tsx'de API_BASE_URL'yi `/api/v1` olarak güncelle**~~ ✅ **TAMAMLANDI** — `/api/v1` proxy kullanılıyor.
2. **CarrierRespond'da fiyat validasyonu ekle** — `if (Number(price) <= 0) return;` + AdminSettings min_offer_price kontrolü. ❌ Yapılmadı
3. ~~**Login'deki demo hesapları bölümünü `import.meta.env.DEV` ile koşullu yap**~~ ✅ **TAMAMLANDI** — `{import.meta.env.DEV && (...)}` eklendi.
4. ~~**RegisterCarrier'a KVKK/kullanım koşulları checkbox'ı ekle**~~ ✅ **TAMAMLANDI** — `termsAccepted` state + checkbox + KVKK metni.
5. ~~**alert() çağrılarını toast() ile değiştir**~~ ✅ **TAMAMLANDI** — RegisterUser ve RegisterCarrier'da `useToast` kullanılıyor.
6. ~~**Login'e "Şifremi Unuttum" link placeholder'ı ekle"**~~ ✅ **TAMAMLANDI** — ForgotPassword.tsx mevcut, `/sifremi-unuttum` rotası çalışıyor.
7. ~~**RegisterUser şifre kurallarını RegisterCarrier ile eşitle**~~ ✅ **TAMAMLANDI** — Backend `validatePassword`: 8 karakter + büyük harf + rakam (kayıt ve şifre sıfırlamada).
8. ~~**Backend şifre validasyonu ekle**~~ ✅ **TAMAMLANDI** — `src/utils/validatePassword.ts` oluşturuldu; CustomerController, CarrierAuthController ve AuthController'a eklendi.

### Kısa Vadeli (1-2 sprint, orta karmaşıklık)

1. ~~**OfferRequestForm Step 3'ü gerçek backend API'ye bağla**~~ ✅ **TAMAMLANDI** — `POST /api/v1/shipments/` entegrasyonu tamamlandı; Step 3 gerçek API ile çalışıyor.
